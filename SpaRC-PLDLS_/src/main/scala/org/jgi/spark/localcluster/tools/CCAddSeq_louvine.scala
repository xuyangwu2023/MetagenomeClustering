...
/**
  ...
  * @DESCRIPTION
  */
object CCAddSeq_louvine extends App with LazyLogging {
  ...
  def parse_command_line(args: Array[String]): Option[Config] = {
    val parser = new scopt.OptionParser[Config]("CCAddSeq_louvine") {
      head("CCAddSeq_louvine", Utils.VERSION)
  ...
  def run(config: Config, sc: SparkContext): Unit = {
    ...
    val ccRDD_louvine =( if (config.n_partition > 0)
      sc.textFile(config.cc_file, minPartitions = config.n_partition)
    else
      sc.textFile(config.cc_file))
      .map(_.drop(1).split(",|:"))
      .map (x =>(x(0).toInt,x(2).toInt ))//.flatMap(x => x)
    ccRDD_louvine.take(5).foreach(print)
//    (4973640,{community:449360,communitySigmaTot:72246,internalWeight:0,nodeWeight:2152})
    ...
    val resultRDD_louvine = ccRDD_louvine.join(readsRDD_louvine).map(_._2).map(x => x._2 + "\t" + x._1.toString).coalesce(config.num_output, shuffle = false)
  ...
  override def main(args: Array[String]) {
    ...
    options match {
      case Some(_) =>
        ...
        val conf = new SparkConf().setAppName("Spark CCAddSeq_louvine")
