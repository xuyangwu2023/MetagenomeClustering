...
object GraphOverlappingCommunityDetection extends App with LazyLogging {


  case class Config(edge_file: String = "", output: String = "", min_shared_kmers: Int = 2, max_iteration: Int = 10,
                    noOfCommunities:Int=4,n_output_blocks: Int = 180,
  ...
  def parse_command_line(args: Array[String]): Option[Config] = {
    val parser = new scopt.OptionParser[Config]("GraphLPA") {
      ...
      opt[Int]("noOfCommunities").action((x, c) =>
        c.copy(max_iteration = x))
        .text("noOfCommunities for OverlappingCommunityDection")
  ...
  def cc(edgeTuples: RDD[(Int, Int)], config: Config, sqlContext: SQLContext) = {
    logInfo(s"1")
    cc_graphx(edgeTuples, sqlContext, config.max_iteration,config.noOfCommunities)
  }
  ...
  def cc_graphx(edgeTuples: RDD[(Int, Int)], sqlContext: SQLContext, max_iteration: Int,noOfCommunities:Int) = {
    ...
    logInfo(s"4")
    logInfo(s"5")
    val cc = graphxlib.OverlappingCommunityDetection.run(graph, max_iteration,noOfCommunities)
    logInfo(s"6")
    val clusters = cc.vertices.map(x => (x._1.toLong, x._2))
    print(clusters)
    logInfo(s"7")
    clusters
  }
