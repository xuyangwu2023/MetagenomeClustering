...
import net.sparc.graph.{AffinityPropagation, LPA}//调用AffinityPropagation,和LPA
...
object GraphAP extends App with LazyLogging {


  case class Config(edge_file: String = "", output: String = "", min_shared_kmers: Int = 2,
                    max_iteration: Int = 10, damping: Float = 0.5f, scaling: Float = 1f,
                    n_output_blocks: Int = 180, weight: String = "none", convergence_iter: Int = 3,
  ...
  def parse_command_line(args: Array[String]): Option[Config] = {
    val parser = new scopt.OptionParser[Config]("GraphAP") {
      ...
      opt[Double]("damping").action((x, c) =>
        c.copy(damping = x.toFloat)).
        validate(x =>
          if (x >= 0.5 && x <= 1) success
          else failure("damping should be between 0.5 and 1"))
        .text("damping factor")
  ...
  def ap(edgeTuples: RDD[(Int, Int, Float)], config: Config, sqlContext: SQLContext) = {
    val (cc, checkpoint) = AffinityPropagation.run(edgeTuples, sqlContext, config.max_iteration,
      config.damping, config.scaling, config.convergence_iter, checkpoint_dir)
  ...
  protected def run_ap(all_edges: RDD[Array[Int]], config: Config, spark: SparkSession,
                       n_reads: Long) = {
    ...
    val (tmp_clusters, checkpoint) = if (config.weight.toLowerCase == "none") {
      ...
      ap(edgeTuples, config, sqlContext)
    } else {
  ...
  def run(config: Config, spark: SparkSession): Long = {
    ...
    val final_clusters = run_ap(edges, config, spark, n_reads)
