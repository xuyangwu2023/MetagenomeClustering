...
object GraphPSCAN extends App with LazyLogging {


  case class Config(edge_file: String = "",output: String = "",min_shared_kmers: Int = 2, epsilon: Double = 0.5,
                    n_output_blocks: Int = 180,
                    min_reads_per_cluster: Int = 2, max_shared_kmers: Int = 20000, sleep: Int = 0,
                    n_partition: Int = 0)

  def parse_command_line(args: Array[String]): Option[Config] = {
    val parser = new scopt.OptionParser[Config]("GraphPSCAN") {
      head("GraphPSCAN", Utils.VERSION)
  ...
  def cc(edgeTuples: RDD[(Int, Int)], config: Config, sqlContext: SQLContext) = {
    logInfo(s"1")
    cc_graphx(edgeTuples, sqlContext, config.epsilon)
  }
  ...
  def cc_graphx(edgeTuples: RDD[(Int, Int)], sqlContext: SQLContext, epsilon: Double) = {
    ...
    val cc = graphxlib.PSCAN.pscan(graph, epsilon)
    logInfo(s"6")
